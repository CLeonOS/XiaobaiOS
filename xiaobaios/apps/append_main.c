...
int cleonos_app_main(int argc, char **argv, char **envp) {
    ...
    ush_init_state(&sh);
    if (ush_command_bootstrap_state("append", &ctx, &sh, initial_cwd, (u64)sizeof(initial_cwd), &has_context) == 0) {
        return 1;
    }
    ...
    if (ush_can_modify_path(&sh, path) == 0) {
        ush_writeln("append: permission denied");
        return 1;
    }

    if (cleonos_sys_fs_append(path, payload, ush_strlen(payload)) == 0ULL) {
