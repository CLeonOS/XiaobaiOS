...
static void install_print_usage(void) {
    ...
    (void)puts("  install2disk update kernel");
    (void)puts("  install2disk reset-root");
...
static int install_setup_root_account(void) {
    ...
    if (cleonos_sys_fs_stat_size(INSTALL_USER_DB_TARGET) != (u64)strlen(record)) {
        (void)puts("install2disk: root account database size verification failed");
        return 0;
    }
...
static int install_reset_root_account(void) {
    install_stage("reset root account");
    ...
    return install_setup_root_account();
}
...
static int install2disk_run(void) {
    ...
    if (cleonos_sys_disk_format_fat32("XIAOBAIOS") == 0ULL) {
...
int cleonos_app_main(int argc, char **argv, char **envp) {
    ...
    if (argc > 1 && argv != (char **)0 && argv[1] != (char *)0) {
        ...
        if (strcmp(argv[1], "reset-root") == 0 || strcmp(argv[1], "root") == 0) {
            return (install_reset_root_account() != 0) ? 0 : 1;
        }
