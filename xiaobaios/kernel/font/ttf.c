...
clks_bool xiaobaios_ttf_rasterize(const struct xiaobaios_ttf_font *font, u32 codepoint, u32 pixel_height,
    ...
    glyph_px = pixel_height > 12U ? pixel_height - 5U : pixel_height - 2U;
    out->advance = ttf_scale_value(font, (i32)ttf_advance_width(font, glyph_id), glyph_px);
    if (out->advance <= 0) {
    ...
    ascent_px = ttf_scale_value(font, (i32)font->ascent, glyph_px);
    baseline = ascent_px;
    if (baseline < 1) {
        baseline = 1;
    }
    ...
    width = ttf_scale_value(font, (i32)(x_max - x_min), glyph_px) + 2;
    ...
    for (i = 0U; i < point_count; i++) {
        points[i].x = ttf_scale_value(font, points[i].x - (i32)x_min, glyph_px * 4U);
        points[i].y = (baseline * 4) - ttf_scale_value(font, points[i].y, glyph_px * 4U);
    }
...

    {
        ...
        for (yy = 0; yy < height && yy < (i32)XIAOBAIOS_TTF_BITMAP_MAX_H; yy++) {
            ...
            for (xx = 0; xx < width && xx < (i32)XIAOBAIOS_TTF_BITMAP_MAX_W; xx++) {
                ...
                if (samples >= 3U) {
                    out->alpha[yy][xx] = 255U;
                } else if (samples == 2U) {
                    out->alpha[yy][xx] = 176U;
                } else if (samples == 1U) {
                    out->alpha[yy][xx] = 80U;
                } else {
                    out->alpha[yy][xx] = 0U;
