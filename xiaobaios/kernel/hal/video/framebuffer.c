...
#include <xiaobaios/ttf.h>

#include "../../../../clks/kernel/hal/video/psf_font.h"

#define XIAOBAIOS_FB_TTF_CACHE_COUNT 256U
#define XIAOBAIOS_FB_TTY_CELL_W 10U
#define XIAOBAIOS_FB_TTY_CELL_H 20U

struct xiaobaios_fb_ttf_cache_entry {
    clks_bool valid;
    u32 codepoint;
    u32 pixel_height;
...
struct clks_fb_state {
    ...
    struct xiaobaios_fb_ttf_cache_entry ttf_cache[XIAOBAIOS_FB_TTF_CACHE_COUNT];
    u32 glyph_width;
    u32 glyph_height;
    clks_bool ready;
};
...
static void clks_fb_apply_ttf_font(void) {
    clks_fb.glyph_width = XIAOBAIOS_FB_TTY_CELL_W;
    clks_fb.glyph_height = XIAOBAIOS_FB_TTY_CELL_H;
    clks_memset(clks_fb.ttf_cache, 0, sizeof(clks_fb.ttf_cache));
}

static u32 clks_fb_mix_color(u32 fg, u32 bg, u8 alpha) {
...
static struct xiaobaios_ttf_bitmap *clks_fb_ttf_cached_glyph(u32 codepoint, u32 pixel_height) {
    u32 slot = (codepoint ^ (pixel_height * 131U)) % XIAOBAIOS_FB_TTF_CACHE_COUNT;

    if (clks_fb.ttf_font_active == CLKS_FALSE) {
        return CLKS_NULL;
    }

    if (clks_fb.ttf_cache[slot].valid == CLKS_TRUE && clks_fb.ttf_cache[slot].codepoint == codepoint &&
        clks_fb.ttf_cache[slot].pixel_height == pixel_height) {
        return &clks_fb.ttf_cache[slot].bitmap;
    }

    if (xiaobaios_ttf_rasterize(&clks_fb.ttf_font, codepoint, pixel_height, &clks_fb.ttf_cache[slot].bitmap) ==
        CLKS_FALSE) {
        clks_fb.ttf_cache[slot].valid = CLKS_FALSE;
        return CLKS_NULL;
    }

    clks_fb.ttf_cache[slot].valid = CLKS_TRUE;
    clks_fb.ttf_cache[slot].codepoint = codepoint;
    clks_fb.ttf_cache[slot].pixel_height = pixel_height;
    return &clks_fb.ttf_cache[slot].bitmap;
}
...
void clks_fb_draw_codepoint_scaled_clip(u32 x, u32 y, u32 codepoint, u32 fg_rgb, u32 bg_rgb, u32 style_flags,
    ...
    if (clks_fb.ttf_font_active == CLKS_TRUE) {
        struct xiaobaios_ttf_bitmap *bitmap;
        u32 cell_cols = clks_fb_layout_cell_width() * clks_fb_codepoint_width(codepoint);
        u32 cell_rows = clks_fb_cell_height();
        u32 pixel_height = cell_rows * scale_y;
        u32 pen_x;
        u32 pen_y;

        if (pixel_height > XIAOBAIOS_TTF_BITMAP_MAX_H) {
            pixel_height = XIAOBAIOS_TTF_BITMAP_MAX_H;
        }

        bitmap = clks_fb_ttf_cached_glyph(codepoint, pixel_height);
        if (bitmap != CLKS_NULL) {
            u32 bitmap_w = (bitmap->width > 0) ? (u32)bitmap->width : 0U;
            u32 bitmap_h = (bitmap->height > 0) ? (u32)bitmap->height : 0U;
            u32 out_cols_ttf = cell_cols * scale_x;
            u32 out_rows_ttf = cell_rows * scale_y;
            u32 draw_cols_ttf = out_cols_ttf;
            u32 draw_rows_ttf = out_rows_ttf;
            u32 row_ttf;

            if (clip_width > 0U && draw_cols_ttf > clip_width) {
                draw_cols_ttf = clip_width;
            }
            if (clip_height > 0U && draw_rows_ttf > clip_height) {
                draw_rows_ttf = clip_height;
            }
            if (x + draw_cols_ttf > clks_fb.info.width) {
                draw_cols_ttf = clks_fb.info.width - x;
            }
            if (y + draw_rows_ttf > clks_fb.info.height) {
                draw_rows_ttf = clks_fb.info.height - y;
            }

            if (draw_cols_ttf == 0U || draw_rows_ttf == 0U) {
                return;
            }

            if (bitmap->glyph_found == CLKS_FALSE) {
                clks_fb_fill_rect(x, y, draw_cols_ttf, draw_rows_ttf, bg_rgb);
                return;
            }

            clks_fb_fill_rect(x, y, draw_cols_ttf, draw_rows_ttf, bg_rgb);
            pen_x = (bitmap_w < draw_cols_ttf) ? ((draw_cols_ttf - bitmap_w) / 2U) : 0U;
            pen_y = (bitmap_h < draw_rows_ttf) ? ((draw_rows_ttf - bitmap_h) / 2U) : 0U;
            ...
            underline_row = (draw_rows_ttf > scale_y) ? (draw_rows_ttf - (2U * scale_y)) : 0U;

            for (row_ttf = 0U; row_ttf < bitmap_h && row_ttf + pen_y < draw_rows_ttf; row_ttf++) {
                u32 col_ttf;
                u32 dst_y = y + pen_y + row_ttf;
                u32 *dst_row =
                    (u32 *)(void *)(clks_fb.address + ((usize)dst_y * (usize)clks_fb.info.pitch) + ((usize)x * 4U));
                u32 *shadow_row =
                    (clks_fb.shadow_ready == CLKS_TRUE)
                        ? (u32 *)(void *)(clks_fb.shadow + ((usize)dst_y * (usize)clks_fb.info.pitch) +
                                          ((usize)x * 4U))
                        : CLKS_NULL;

                for (col_ttf = 0U; col_ttf < bitmap_w && col_ttf + pen_x < draw_cols_ttf; col_ttf++) {
                    u8 alpha = bitmap->alpha[row_ttf][col_ttf];
                    u32 dst_col = pen_x + col_ttf;
                    u32 color;

                    if (style_bold == CLKS_TRUE && alpha == 0U && col_ttf > 0U) {
                        alpha = bitmap->alpha[row_ttf][col_ttf - 1U];
                    }
                    if (alpha == 0U) {
                        continue;
                    }
                    color = (alpha == 255U) ? fg_rgb : clks_fb_mix_color(fg_rgb, bg_rgb, alpha);
                    if (shadow_row != CLKS_NULL) {
                        shadow_row[dst_col] = color;
                    }
                    dst_row[dst_col] = color;
                }
            }

            if (style_underline == CLKS_TRUE && underline_row < draw_rows_ttf) {
                clks_fb_fill_rect(x, y + underline_row, draw_cols_ttf, scale_y, fg_rgb);
            }
            return;
        }
    }
...
clks_bool clks_fb_load_psf_font(const void *blob, u64 blob_size) {
    ...
    struct xiaobaios_ttf_font parsed_ttf = {0};
    ...
    if (xiaobaios_ttf_parse(font_copy, blob_size, &parsed_ttf) == CLKS_TRUE) {
        if (clks_fb.external_font_blob != CLKS_NULL) {
            clks_kfree(clks_fb.external_font_blob);
        }
        ...
        clks_fb.ttf_font = parsed_ttf;
        clks_fb.ttf_font_active = CLKS_TRUE;
        clks_fb.external_font_active = CLKS_FALSE;
        clks_fb_apply_ttf_font();
        return CLKS_TRUE;
    }
