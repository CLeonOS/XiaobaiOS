...
clks_bool clks_locale_is_valid(const char *locale) {
    ...
    if (clks_strcmp(locale, "zh-CN") == 0 || clks_strcmp(locale, "en-US") == 0) {
        return CLKS_TRUE;
    }
...
void clks_locale_init(void) {
    ...
    clks_locale_copy(clks_locale_value, sizeof(clks_locale_value), "zh-CN");
    ...
    if (data != CLKS_NULL && size > 0ULL) {
        ...
        (void)clks_locale_set("zh-CN", CLKS_TRUE);
    }
