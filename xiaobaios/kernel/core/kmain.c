...
void clks_kernel_main(void) {
    ...
    if (boot_fb != CLKS_NULL) {
        ...
        const char *tty_font_path = "/system/fonts/tty-en-US.psf";
        const char *locale = clks_locale_current();

        if (locale != CLKS_NULL && clks_strcmp(locale, "zh-CN") == 0) {
            tty_font_path = "/system/fonts/tty-zh-CN.ttf";
        }

        tty_psf_blob = clks_fs_read_all(tty_font_path, &tty_psf_size);
        ...
        if (tty_psf_blob != CLKS_NULL && clks_fb_load_psf_font(tty_psf_blob, tty_psf_size) == CLKS_TRUE) {
            ...
            clks_log(CLKS_LOG_INFO, "TTY", "EXTERNAL TTY FONT LOADED");
            clks_log(CLKS_LOG_INFO, "TTY", tty_font_path);
            clks_log_hex(CLKS_LOG_INFO, "TTY", "FONT_SIZE", tty_psf_size);
        } else {
            clks_log(CLKS_LOG_WARN, "TTY", "EXTERNAL TTY FONT LOAD FAILED, USING BUILTIN");
