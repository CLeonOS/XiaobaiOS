...
#define CLKS_SHELL_ANSI_BLUE_BOLD "\x1B[1;34m"
#define CLKS_SHELL_ANSI_CYAN_BOLD "\x1B[1;36m"
#define CLKS_SHELL_ANSI_CLEAR "\x1B[2J"
#define CLKS_SHELL_ANSI_HOME "\x1B[H"
...
static char clks_shell_user_name[CLKS_SHELL_NAME_MAX] = "root";
static u64 clks_shell_user_uid = 0ULL;
static u64 clks_shell_user_gid = 0ULL;
...
static void clks_shell_write_u64_dec(u64 value) {
    char digits[21];
    usize len = 0U;

    if (value == 0ULL) {
        clks_shell_write_char('0');
        return;
    }

    while (value > 0ULL && len < sizeof(digits)) {
        digits[len++] = (char)('0' + (value % 10ULL));
        value /= 10ULL;
    }

    while (len > 0U) {
        len--;
        clks_shell_write_char(digits[len]);
    }
}
...
static void clks_shell_prompt(void) {
    clks_shell_write("[");
    clks_shell_write(CLKS_SHELL_ANSI_GREEN_BOLD);
    clks_shell_write(clks_shell_user_name);
    clks_shell_write(CLKS_SHELL_ANSI_RESET);
    clks_shell_write("@");
    clks_shell_write(CLKS_SHELL_ANSI_BLUE_BOLD);
    clks_shell_write(CLKS_SHELL_HOSTNAME_TEXT);
    clks_shell_write(CLKS_SHELL_ANSI_RESET);
    clks_shell_write(":");
    clks_shell_write(CLKS_SHELL_ANSI_CYAN_BOLD);
    clks_shell_write(clks_shell_cwd);
    clks_shell_write(CLKS_SHELL_ANSI_RESET);
    clks_shell_write("] ");
    clks_shell_write(clks_shell_user_uid == 0ULL ? "# " : "$ ");
}

static clks_bool clks_shell_is_root(void) {
    return (clks_shell_user_uid == 0ULL) ? CLKS_TRUE : CLKS_FALSE;
}
...
static void clks_shell_render_line(void) {
    ...
    clks_shell_write_char(' ');
    for (i = 0U; i < clks_shell_rendered_len + 8U; i++) {
        clks_shell_write_char(' ');
    }
    clks_shell_write_char('\r');
    clks_shell_prompt();
    ...
    backspaces = (clks_shell_line_len > clks_shell_cursor) ? (clks_shell_line_len - clks_shell_cursor) : 0U;

    for (i = 0U; i < backspaces; i++) {
...
static void clks_shell_move_cursor_left_word(void) {
    while (clks_shell_cursor > 0U && clks_shell_is_space(clks_shell_line[clks_shell_cursor - 1U]) == CLKS_TRUE) {
        clks_shell_cursor--;
    }

    while (clks_shell_cursor > 0U && clks_shell_is_space(clks_shell_line[clks_shell_cursor - 1U]) == CLKS_FALSE) {
        clks_shell_cursor--;
    }
...
static void clks_shell_move_cursor_right_word(void) {
    while (clks_shell_cursor < clks_shell_line_len &&
           clks_shell_is_space(clks_shell_line[clks_shell_cursor]) == CLKS_TRUE) {
        clks_shell_cursor++;
    }

    while (clks_shell_cursor < clks_shell_line_len &&
           clks_shell_is_space(clks_shell_line[clks_shell_cursor]) == CLKS_FALSE) {
        clks_shell_cursor++;
    }
...
static void clks_shell_delete_prev_word(void) {
    usize start = clks_shell_cursor;
    usize tail_len;
    ...
    while (start > 0U && clks_shell_is_space(clks_shell_line[start - 1U]) == CLKS_TRUE) {
        start--;
    }

    while (start > 0U && clks_shell_is_space(clks_shell_line[start - 1U]) == CLKS_FALSE) {
        start--;
    }

    tail_len = clks_shell_line_len - clks_shell_cursor;
    clks_memmove(clks_shell_line + start, clks_shell_line + clks_shell_cursor, tail_len + 1U);

    clks_shell_cursor = start;
    clks_shell_line_len = clks_strlen(clks_shell_line);
    clks_shell_render_line();
}

static void clks_shell_kill_to_end(void) {
    usize i;
    ...
    clks_shell_line_len = clks_shell_cursor;
    clks_shell_render_line();
}

static void clks_shell_kill_to_bol(void) {
    usize i;
    ...
    for (i = 0U; i <= clks_shell_line_len; i++) {
        clks_shell_line[i] = clks_shell_line[i + clks_shell_cursor];
    }
...
static void clks_shell_redraw_clean(void) {
    usize i;
...
static clks_bool clks_shell_cmd_help(void) {
    ...
    clks_shell_writeln("  ansi / ansitest / elfrunner / memc");
...
static clks_bool clks_shell_cmd_whoami(void) {
    clks_shell_writeln(clks_shell_user_name);
    return CLKS_TRUE;
}

static clks_bool clks_shell_cmd_id(void) {
    clks_shell_writeln("id:");
    clks_shell_write("  uid=");
    clks_shell_write_u64_dec(clks_shell_user_uid);
    clks_shell_write("(");
    clks_shell_write(clks_shell_user_name);
    clks_shell_writeln(")");
    clks_shell_write("  gid=");
    clks_shell_write_u64_dec(clks_shell_user_gid);
    clks_shell_writeln("(root)");
    return CLKS_TRUE;
}

static clks_bool clks_shell_cmd_su(const char *arg) {
    const char *target = arg;

    if (target == CLKS_NULL || target[0] == '\0' || clks_shell_streq(target, "root") == CLKS_TRUE) {
        clks_shell_copy_line(clks_shell_user_name, sizeof(clks_shell_user_name), "root");
        clks_shell_user_uid = 0ULL;
        clks_shell_user_gid = 0ULL;
        clks_shell_writeln("switched to root");
        return CLKS_TRUE;
    }

    clks_shell_writeln("su: only the default root account exists");
    return CLKS_FALSE;
}
...
static clks_bool clks_shell_cmd_write(const char *arg) {
    ...
    if (clks_shell_can_modify_path(abs_path) == CLKS_FALSE) {
        clks_shell_writeln("write: permission denied");
        return CLKS_FALSE;
    }
...
static clks_bool clks_shell_cmd_append(const char *arg) {
    ...
    if (clks_shell_can_modify_path(abs_path) == CLKS_FALSE) {
        clks_shell_writeln("append: permission denied");
        return CLKS_FALSE;
    }
...
static clks_bool clks_shell_cmd_cp(const char *arg) {
    ...
    if (clks_shell_can_modify_path(dst_path) == CLKS_FALSE) {
        clks_shell_writeln("cp: permission denied for destination");
        return CLKS_FALSE;
    }
...
static clks_bool clks_shell_cmd_mv(const char *arg) {
    ...
    if (clks_shell_can_modify_path(src_path) == CLKS_FALSE ||
        clks_shell_can_modify_path(dst_path) == CLKS_FALSE) {
        clks_shell_writeln("mv: permission denied");
        return CLKS_FALSE;
    }
...
static clks_bool clks_shell_cmd_rm(const char *arg) {
    ...
    if (clks_shell_can_modify_path(path) == CLKS_FALSE) {
        clks_shell_writeln("rm: permission denied");
        return CLKS_FALSE;
    }
...
static clks_bool clks_shell_cmd_exec(const char *arg) {
    ...
    if (clks_exec_spawn_path(path, &pid) == CLKS_TRUE) {
        clks_shell_writeln("exec: spawn queued");
        clks_shell_write("  PATH: ");
        clks_shell_writeln(path);
        clks_shell_print_kv_hex("  PID", pid);
        return CLKS_TRUE;
    }

    if (clks_exec_run_path(path, &status) == CLKS_TRUE && status == 0ULL) {
        clks_shell_writeln("exec: run accepted");
        return CLKS_TRUE;
    }
...
static clks_bool clks_shell_cmd_ps(void) {
    u64 count;
    u64 i;

    count = clks_exec_proc_count();
    clks_shell_writeln("ps:");
    clks_shell_print_kv_hex("  PROC_COUNT", count);

    for (i = 0ULL; i < count; i++) {
        u64 pid = 0ULL;
        struct clks_exec_proc_snapshot snap;

        if (clks_exec_proc_pid_at(i, &pid) == CLKS_FALSE) {
            continue;
        }
        ...
        clks_shell_write("  [");
        clks_shell_write_hex_u64(snap.pid);
        clks_shell_write("] ");
        clks_shell_write(snap.path);
        clks_shell_write(" state=");
        clks_shell_write_hex_u64(snap.state);
        clks_shell_write(" runtime=");
        clks_shell_write_hex_u64(snap.runtime_ticks);
        clks_shell_write(" mem=");
        clks_shell_write_hex_u64(snap.mem_bytes);
        clks_shell_write(" tty=");
        clks_shell_write_hex_u64(snap.tty_index);
        clks_shell_write_char('\n');
    }
...
static clks_bool clks_shell_cmd_kelfstat(void) {
    clks_shell_writeln("kelfstat:");
    clks_shell_print_kv_hex("  APP_COUNT", clks_kelf_count());
    clks_shell_print_kv_hex("  TOTAL_RUNS", clks_kelf_total_runs());
    return CLKS_TRUE;
}
...
static clks_bool clks_shell_cmd_taskstat(void) {
    ...
    for (i = 0U; i < sched.task_count; i++) {
        ...
        clks_shell_write(" state=");
        clks_shell_write(clks_shell_exec_state_name((u64)task->state));
        clks_shell_write("(");
...
static clks_bool clks_shell_cmd_ansi(void) {
    clks_shell_writeln("\x1B[1;36mansi color demo\x1B[0m");
    clks_shell_writeln("  \x1B[30mblack\x1B[0m \x1B[31mred\x1B[0m \x1B[32mgreen\x1B[0m \x1B[33myellow\x1B[0m");
    clks_shell_writeln("  \x1B[34mblue\x1B[0m \x1B[35mmagenta\x1B[0m \x1B[36mcyan\x1B[0m \x1B[37mwhite\x1B[0m");
    clks_shell_writeln("  \x1B[90mbright-black\x1B[0m \x1B[91mbright-red\x1B[0m \x1B[92mbright-green\x1B[0m \x1B[93mbright-yellow\x1B[0m");
    clks_shell_writeln("  \x1B[94mbright-blue\x1B[0m \x1B[95mbright-magenta\x1B[0m \x1B[96mbright-cyan\x1B[0m \x1B[97mbright-white\x1B[0m");
    return CLKS_TRUE;
}

static clks_bool clks_shell_cmd_ansitest(void) {
    u64 i;

    clks_shell_writeln("\x1B[1;96mANSI test suite\x1B[0m");
    clks_shell_writeln("styles: \x1B[1mbold\x1B[0m  \x1B[7minverse\x1B[0m  \x1B[4munderline\x1B[0m");
    clks_shell_writeln("16-color demo:");
    (void)clks_shell_cmd_ansi();

    clks_shell_writeln("256-color palette (0..255):");
    for (i = 0ULL; i < 256ULL; i++) {
        ...
        if (v == 0ULL) {
            ...
            while (v > 0ULL && digits < (u64)sizeof(rev)) {
                rev[digits++] = (char)('0' + (v % 10ULL));
                v /= 10ULL;
            }

            for (j = 0ULL; j < digits; j++) {
                num[j] = rev[digits - 1ULL - j];
            }
            num[digits] = '\0';
        }
        ...
        clks_shell_write(seq);
        if ((i % 32ULL) == 31ULL) {
            clks_shell_write_char('\n');
        }
    }
    clks_shell_write_char('\n');
    clks_shell_writeln("truecolor demo:");
    clks_shell_writeln("  \x1B[38;2;255;64;64mRGB(255,64,64)\x1B[0m  \x1B[38;2;64;255;64mRGB(64,255,64)\x1B[0m  \x1B[38;2;64;128;255mRGB(64,128,255)\x1B[0m");
    clks_shell_writeln("cursor control demo:");
    clks_shell_write("  0123456789");
    clks_shell_write("\x1B[5D");
    clks_shell_write("\x1B[93m<OK>\x1B[0m");
    clks_shell_write_char('\n');
    clks_shell_write("  save");
    clks_shell_write("\x1B[s");
    clks_shell_write("....");
    clks_shell_write("\x1B[u");
    clks_shell_write("\x1B[92m<restore>\x1B[0m");
    clks_shell_write_char('\n');
    clks_shell_writeln("erase-line demo:");
    clks_shell_write("  left|right-to-clear");
    clks_shell_write("\x1B[14D\x1B[K");
    clks_shell_write_char('\n');
    clks_shell_writeln("ansitest done");
    return CLKS_TRUE;
}

static clks_bool clks_shell_cmd_elfrunner(void) {
    clks_shell_writeln("[KAPP][ELFRUNNER] elfrunner.elf online");
    return CLKS_TRUE;
}

static clks_bool clks_shell_cmd_memc(void) {
    clks_shell_writeln("[KAPP][MEMC] memc.elf online");
    return CLKS_TRUE;
}
...
static void clks_shell_execute_line(const char *line) {
    ...
    } else if (clks_shell_streq(cmd, "ps") == CLKS_TRUE) {
        success = clks_shell_cmd_ps();
    } else if (clks_shell_streq(cmd, "kelfstat") == CLKS_TRUE) {
        success = clks_shell_cmd_kelfstat();
    ...
    } else if (clks_shell_streq(cmd, "ansi") == CLKS_TRUE || clks_shell_streq(cmd, "color") == CLKS_TRUE) {
        success = clks_shell_cmd_ansi();
    } else if (clks_shell_streq(cmd, "ansitest") == CLKS_TRUE) {
        success = clks_shell_cmd_ansitest();
    } else if (clks_shell_streq(cmd, "elfrunner") == CLKS_TRUE) {
        success = clks_shell_cmd_elfrunner();
    } else if (clks_shell_streq(cmd, "memc") == CLKS_TRUE) {
        success = clks_shell_cmd_memc();
...
static void clks_shell_handle_char(char ch) {
    ...
    if (ch == CLKS_KEY_SHIFT_LEFT) {
        clks_shell_move_cursor_left_word();
        return;
    }
    ...
    if (ch == CLKS_KEY_SHIFT_RIGHT) {
        clks_shell_move_cursor_right_word();
        return;
    }
    ...
    if (ch == CLKS_KEY_SHIFT_HOME) {
        clks_shell_kill_to_bol();
        return;
    }
    ...
    if (ch == CLKS_KEY_SHIFT_END) {
        clks_shell_kill_to_end();
        return;
    }

    if (ch == CLKS_KEY_CTRL_A) {
        clks_shell_cursor = 0U;
        clks_shell_render_line();
        return;
    }

    if (ch == CLKS_KEY_CTRL_E) {
        clks_shell_cursor = clks_shell_line_len;
        clks_shell_render_line();
        return;
    }

    if (ch == CLKS_KEY_CTRL_L) {
        clks_shell_redraw_clean();
        return;
    }

    if (ch == CLKS_KEY_CTRL_U) {
        clks_shell_kill_to_bol();
        return;
    }

    if (ch == CLKS_KEY_CTRL_K) {
        clks_shell_kill_to_end();
        return;
    }

    if (ch == CLKS_KEY_CTRL_W) {
        clks_shell_delete_prev_word();
        return;
    }

    if (ch == CLKS_KEY_CTRL_D) {
        if (clks_shell_line_len == 0U) {
            clks_shell_writeln("exit");
    ...
    if (ch == CLKS_KEY_CTRL_C) {
        clks_shell_writeln("^C");
        clks_shell_reset_line();
        clks_shell_history_cancel_nav();
        clks_shell_prompt();
        return;
