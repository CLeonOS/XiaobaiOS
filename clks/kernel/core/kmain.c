...
static void clks_task_usrd(u64 tick) {
    ...
    clks_exec_tick(tick);
#if CLKS_CFG_USERLAND_AUTO_EXEC
...
void clks_kernel_main(void) {
    ...
    clks_log(CLKS_LOG_INFO, "BOOT", "XIAOBAIOS START");
...
#if CLKS_CFG_KELF
    clks_log(CLKS_LOG_INFO, "KELF", "KELF ENABLED BY DEFAULT");
#endif
...
#if CLKS_CFG_SYSCALL_SERIAL_LOG
    clks_log(CLKS_LOG_INFO, "CFG", "SYSCALL SERIAL LOGS ENABLED");
#endif

#if CLKS_CFG_SYSCALL_USERID_SERIAL_LOG
    clks_log(CLKS_LOG_INFO, "CFG", "SYSCALL USER_ID SERIAL LOGS ENABLED");
#endif
